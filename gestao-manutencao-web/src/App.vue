<script setup>
    ...
    async function buscarClientes() {
        try {
            const resposta = await fetch('https://gestao-manutencao.onrender.com/api/Cliente');
	...
	async function salvarCliente() {
		try {
			...
			const urlDaApi = isEdicao
				? `https://gestao-manutencao.onrender.com/api/Cliente/${pacoteCliente.id}`
				: 'https://gestao-manutencao.onrender.com/api/Cliente';
	...
	async function deletarCliente(id, nome) {
		if (confirm(`Atenção: Deseja realmente excluir o cliente ${nome}?`)) {
			try {
				const resposta = await fetch(`https://gestao-manutencao.onrender.com/api/Cliente/${id}`, {
					method: 'DELETE'
				});
    ...
    async function buscarOrdensDeServico() {
        try {
            const resposta = await fetch('https://gestao-manutencao.onrender.com/api/OrdemDeServico');
    ...
    async function salvarOS() {
        try {
            ...
            const urlDaApi = isEdicao
                ? `https://gestao-manutencao.onrender.com/api/OrdemDeServico/${pacoteOS.id}`
                : 'https://gestao-manutencao.onrender.com/api/OrdemDeServico';
    ...
    async function deletarOS(id) {
        if (confirm(`Atenção: Tem certeza que deseja excluir a OS #${id}? Essa ação não tem volta.`)) {
            try {
                const resposta = await fetch(`https://gestao-manutencao.onrender.com/api/OrdemDeServico/${id}`, {
                    method: 'DELETE'
                });
